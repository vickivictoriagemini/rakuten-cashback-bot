...
export default function MomoPage() {
  const [targets, setTargets] = useState<MomoTarget[]>([])
  ...
  const fetchTargets = async () => {
    const res = await fetch('/api/momo/targets')
  ...
  const handleToggle = async (target: MomoTarget) => {
    await fetch(`/api/momo/targets/${target.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch(`/api/momo/targets/${id}`, { method: 'DELETE' })
  ...
  const toggleHistory = async (targetId: number) => {
    ...
    if (!history[targetId]) {
      const res = await fetch(`/api/momo/history/${targetId}`)
      if (res.ok) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="glass-panel animate-fade-in delay-1">
        ...
        <p className="text-secondary" style={{ marginBottom: '20px', lineHeight: 1.6 }}>
          Paste a Momo product URL. Your Pi 5 will check the price hourly and alert you when it drops below your target.
        </p>
        ...
        <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div className="input-group" style={{ marginBottom: 0 }}>
            <label>Momo Product URL</label>
            <input type="url" placeholder="https://momo.tw/product/..." value={url} onChange={e => setUrl(e.target.value)} required />
          </div>
      ...
      <div className="glass-panel animate-fade-in delay-2">
        ...
        {targets.length === 0 ? (
          <p className="text-secondary" style={{ textAlign: 'center', padding: '40px' }}>
            No products yet. Add a Momo product URL above to start monitoring!
          </p>
        ) : (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                              <LineChart data={[...history[target.id]].reverse().map(h => ({
                                ...
                                <Tooltip 
                                  ...
                                  itemStyle={{ color: '#e52b50', fontWeight: 'bold' }}
                                ...
                                <Line 
                                  ...
                                  stroke="#e52b50" 
                                  strokeWidth={3}
                                  dot={{ fill: '#e52b50', r: 4, strokeWidth: 2, stroke: '#fff' }}
                                  activeDot={{ r: 6, fill: '#e52b50', stroke: '#fff' }}
                                />
                              </LineChart>
                            </ResponsiveContainer>
                          </div>
      ...
      <div className="glass-panel animate-fade-in delay-3" style={{ borderColor: 'rgba(251,191,36,0.3)' }}>
        ...
        <p className="text-secondary" style={{ marginBottom: '16px', lineHeight: 1.6 }}>
          The scraper runs on your Pi 5 at home (using your residential IP to bypass Momo's bot protection). Run these once to set it up:
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            ...
            'cd ~/rakuten && npx tsx scripts/momo-scraper.ts',
            '',
            '# 5. Add to crontab (run hourly)',
            'crontab -e',
            '# Add: 0 * * * * cd /home/pi/rakuten && npx tsx scripts/momo-scraper.ts >> /home/pi/momo.log 2>&1',
          ].map((line, i) => (
            <code key={i} style={{
              display: 'block',
      ...
      {selectedScreenshot && (
        ...
        >
          <div style={{ position: 'relative', maxWidth: '90vw', maxHeight: '90vh', background: '#fff', borderRadius: '12px', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <img src={selectedScreenshot} alt="Momo Screenshot" style={{ display: 'block', maxWidth: '100%', maxHeight: '100%', objectFit: 'contain' }} />
            <button 
              onClick={() => setSelectedScreenshot(null)}
